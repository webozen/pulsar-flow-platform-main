...
> A paused workflow that will automatically resume after a specified duration.
...
Kestra persists the execution state in its Postgres backend. After 3 days, execution continues with the next task. This survives Kestra restarts.

## Re-validation after resume
After any pause, Flowcore workflows re-query Open Dental to check if the situation resolved:

```yaml
- id: recheck_appointment
  type: io.kestra.plugin.jdbc.mysql.Query
  sql: |
    SELECT COUNT(*) as has_appt FROM appointment
    WHERE PatNum = {{ trigger.row.PatNum }} AND AptStatus = 1 AND AptDateTime > NOW()
```
